...
public class AppDataUpgrader extends IUpgrader {
    ...
    public static class AppDataUpgraderPackGzTask extends Task {
        ...
        public void executeTask(boolean areDependTasksSucceeded) throws Exception {
            ...
            executePack200Unpack(tempFile, f);
        ...
        /** Unpack .pack.gz file. Pack200 was removed in Java 14+; use reflection. */
        private static void executePack200Unpack(File packGz, File jarOut) throws IOException {
            int major;
            try {
                major = Integer.parseInt(System.getProperty("java.version").split("\\.")[0]);
            } catch (NumberFormatException e) {
                major = 8;
            }
            if (major >= 14) {
                throw new IOException("Pack200 update format not supported on Java " + major);
            }
            try (GZIPInputStream in = new GZIPInputStream(FileUtils.openInputStream(packGz));
                    JarOutputStream jos = new JarOutputStream(FileUtils.openOutputStream(jarOut))) {
                Class<?> pack200 = Class.forName("java.util.jar.Pack200");
                Object unpacker = pack200.getMethod("newUnpacker").invoke(null);
                pack200.getMethod("unpack", GZIPInputStream.class, JarOutputStream.class).invoke(unpacker, in, jos);
            } catch (ReflectiveOperationException e) {
                throw new IOException("Pack200 unpack failed: " + e.getMessage(), e);
            }
        }
