...
public final class Config implements Cloneable {
    ...
    /** Download speed limit level (0~42). Ported from PCL: ToolDownloadSpeed.
     *  0=0.1M/s, 14=1.5M/s, 31=10M/s, 41=20M/s, 42=unlimited */
    ...
    public Config() {
        ...
        logLines = 100;
        sourceMode = 0; // Default: China/Auto (matches PCL ToolDownloadVersion=0)
        downloadSpeed = 42; // Default: unlimited (matches PCL ToolDownloadSpeed=42)
    ...
    /**
     * Download source selection mode. Equivalent to PCL's ToolDownloadVersion.
     * 0 = China/Auto (prefer BMCLAPI, fallback to Mojang)
     * 1 = International (prefer Mojang, fallback to BMCLAPI)
     * 2 = Fastest (parallel test, use fastest)
     */
    ...
    public DownloadSourceManager.SourceMode getSourceModeEnum() {
        return DownloadSourceManager.SourceMode.fromCode(sourceMode);
    }

    public void setSourceModeEnum(DownloadSourceManager.SourceMode mode) {
        this.sourceMode = mode.getCode();
        Settings.save();
    }

    /**
     * Get download speed limit level (0~42).
     * Ported from PCL: ModNet.vb NetTaskSpeedLimitHigh
     * 
     * Mapping:
     *   0~14   → (v+1) * 0.1 MB/s  (0.1~1.5 MB/s)
     *   15~31  → (v-11) * 0.5 MB/s (2.0~10.0 MB/s)
     *   32~41  → (v-21) * 1.0 MB/s (11~20 MB/s)
     *   42     → unlimited
     */
    ...
    public void setDownloadSpeed(int downloadSpeed) {
        this.downloadSpeed = Math.max(0, Math.min(42, downloadSpeed));
        Settings.save();
    }

    /** Convert speed level to bytes/sec limit. -1 means unlimited. */
    public long getDownloadSpeedBytesPerSec() {
        int v = downloadSpeed;
        if (v <= 14) {
            return (long) ((v + 1) * 0.1 * 1024 * 1024);
        } else if (v <= 31) {
            return (long) ((v - 11) * 0.5 * 1024 * 1024);
        } else if (v <= 41) {
            return (long) ((v - 21) * 1.0 * 1024 * 1024);
        } else {
            return -1; // unlimited
        }
    }

    /** Human-readable speed label for UI. */
    public String getDownloadSpeedLabel() {
        int v = downloadSpeed;
        if (v <= 14) {
            return String.format("%.1f MB/s", (v + 1) * 0.1);
        } else if (v <= 31) {
            return String.format("%.1f MB/s", (v - 11) * 0.5);
        } else if (v <= 41) {
            return String.format("%d MB/s", v - 21);
        } else {
            return "\u65e0\u9650\u5236"; // "unlimited"
        }
    }
